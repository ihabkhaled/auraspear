    ...
    <div className="space-y-6">
      ...
      {stats && (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
          <KpiCard
            label={t('kpi.totalSuites')}
            value={String(stats.totalSuites ?? 0)}
          ...
          <KpiCard
            ...
            value={
              stats.avgScore !== null && stats.avgScore !== undefined
                ? `${(Number(stats.avgScore) * 100).toFixed(1)}%`
                : '-'
            }
          ...
          />
          <KpiCard
            label={t('kpi.pendingRuns')}
            value={String(stats.pendingRuns ?? 0)}
            icon={<Clock className="h-4 w-4" />}
            accentColor={undefined}
          />
          <KpiCard
            label={t('kpi.completedRuns')}
            value={String(stats.completedRuns ?? 0)}
            icon={<CheckCircle className="h-4 w-4" />}
            accentColor={undefined}
          />
          <KpiCard
            label={t('kpi.failedRuns')}
          ...
          {showCreateSuite && (
            <div className="bg-muted/50 grid grid-cols-1 gap-3 rounded-lg border p-4 sm:grid-cols-3">
              <Input
                ...
                value={newSuiteName}
                onChange={e => setNewSuiteName(e.currentTarget.value)}
              />
              <Textarea
                placeholder={t('suites.descPlaceholder')}
                value={newSuiteDesc}
                onChange={e => setNewSuiteDesc(e.currentTarget.value)}
                rows={1}
              />
              <div className="flex gap-2">
                <Button
                  ...
                  disabled={isCreatingSuite || newSuiteName.trim().length === 0}
                >
                  {t('suites.save')}
                </Button>
        ...
        <div className="space-y-4">
          ...
          {showStartRun && (
            <div className="bg-muted/50 grid grid-cols-1 gap-3 rounded-lg border p-4 sm:grid-cols-4">
              {/* Suite dropdown */}
              <Select
                value={runSuiteId || 'none'}
                onValueChange={v => setRunSuiteId(v === 'none' ? '' : v)}
              >
                <SelectTrigger>
              ...
              {/* Model name */}
              <Input
                placeholder={t('runs.modelPlaceholder')}
                value={runModel}
                onChange={e => setRunModel(e.currentTarget.value)}
              />
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  disabled={isStartingRun || !runSuiteId || !runModel.trim()}
                >
                  {t('runs.execute')}
                </Button>
