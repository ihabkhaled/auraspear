...
export function TranscriptKpis({
  ...
  return (
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
      <KpiCard
        label={t('kpi.threads')}
        value={Number(stats?.totalThreads ?? 0).toLocaleString()}
      ...
      />
      <KpiCard
        label={t('kpi.auditLogs')}
        value={Number(stats?.totalAuditLogs ?? 0).toLocaleString()}
        accentColor={undefined}
        icon={<ScrollText className="h-4 w-4" />}
      />
      <KpiCard
        label={t('kpi.onHold')}
        value={Number(stats?.threadsOnHold ?? 0).toLocaleString()}
        accentColor={undefined}
        icon={<Lock className="h-4 w-4" />}
      />
      <KpiCard
        label={t('kpi.redacted')}
        value={Number(stats?.threadsRedacted ?? 0).toLocaleString()}
        accentColor={undefined}
